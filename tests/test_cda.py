import numpy as np
import pytest
from ledalabpy.analyze.cda import continuous_decomposition_analysis
from ledalabpy.data_models import EDAData, EDASetting

def test_cda_synthetic_data():
    """Test CDA with synthetic data."""
    ...
    settings = EDASetting()
    analysis = continuous_decomposition_analysis(eda_data, settings, optimize=0)
    
    assert analysis.phasic_data is not None
    assert analysis.tonic_data is not None
    assert analysis.driver is not None
    
    assert np.corrcoef(analysis.tonic_data, tonic)[0, 1] > 0.8
