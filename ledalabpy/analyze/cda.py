...
def continuous_decomposition_analysis(data: EDAData, settings: Optional[EDASetting] = None,
    ...
    if hasattr(settings, 'peak_detection_threshold') and settings.peak_detection_threshold is not None:
        threshold = settings.peak_detection_threshold
    else:
        # Default threshold matching MATLAB's sensitivity
    ...
    if len(max_idx) > 0:
        ...
        if len(peaks) > 0:
            try:
                ...
                print(f"Warning: Error accessing time data for peaks/onsets: {e}")
                analysis.impulse_onset = np.array([])
                analysis.impulse_peak_time = np.array([])

            if hasattr(settings, 'amplitude_scaling_cda') and settings.amplitude_scaling_cda is not None:
                amp_scale = settings.amplitude_scaling_cda
            else:
                amp_scale = 400.0  # Default scaling that works across datasets
